...
if (isset($_POST["title"]) and isset($_POST["discription"]) and isset($_POST["content"]) and isset($_POST["date"])) {
    ...
    if ($_POST["title"] == "" or $_POST["discription"] == "" or $_POST["content"] == "" or $_POST["date"] == "") {
        ...
        $connection->setQuery("SELECT * FROM posts WHERE title='" . $_POST["title"] . "'");
        ...
        if (!is_null($result)) {
            ...
            $connection->setQuery("insert into posts (title, discription, content, date, user) values ('$title', '$discription', '$content', '$date', '$user')");
